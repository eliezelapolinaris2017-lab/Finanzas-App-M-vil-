...
export default function IncomesScreen(){
  ...
  const save = async ()=>{
    if(!date) return;
    const rec = { id: Math.random().toString(36).slice(2), date, client, method, amount: Number(amount||0), ref };
    await atomicAppend(user.uid, 'incomesDaily', rec);
    setClient(''); setAmount(''); setRef('');
  };

  const list = useMemo(()=> (cloudState?.incomesDaily||[]).slice().sort((a,b)=> (a.date<b.date?1:-1)), [cloudState]);

  return (
    <View style={{ flex:1, padding:14 }}>
      <Text style={{ color:'#F2F3F5', fontWeight:'700', marginBottom:6 }}>Nuevo ingreso</Text>
      <View style={{ backgroundColor:'#111317', borderColor:'#262c34', borderWidth:1, borderRadius:12, padding:10, marginBottom:12 }}>
        ...
        <TextInput placeholder="Cliente/Origen" placeholderTextColor="#8a8f99" value={client} onChangeText={setClient}
          style={{ color:'#F2F3F5', borderBottomWidth:1, borderBottomColor:'#262c34', paddingVertical:6, marginBottom:8 }}/>
        <TextInput placeholder="Método (Efectivo/Tarjeta/...)" placeholderTextColor="#8a8f99" value={method} onChangeText={setMethod}
          style={{ color:'#F2F3F5', borderBottomWidth:1, borderBottomColor:'#262c34', paddingVertical:6, marginBottom:8 }}/>
      ...
      <FlatList
        ...
        renderItem={({item})=>(
          <View style={{ borderBottomColor:'#262c34', borderBottomWidth:1, paddingVertical:10 }}>
            <Text style={{ color:'#F2F3F5' }}>{item.date} · {item.client} · {item.method} · {item.ref||'—'}</Text>
